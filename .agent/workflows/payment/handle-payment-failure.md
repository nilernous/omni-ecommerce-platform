...
name: Handle Payment Charge Failure

description: Handles failed payment attempts, releases reserved stock, and notifies customer.
...
# Workflow: Handle Payment Charge Failure

---

# 1. Purpose

Handles failed payment attempts, releases reserved stock, and notifies customer.
...
- Core process execution for Handle Payment Charge Failure.
...
Handle Payment Charge Failure is initiated via incoming client API request or internal message event.
...
**Failure Logging**: Records `status = FAILED` and decline reason in `payment_db` (`PAY-BR-14`).

---

## Step 2

**Event Emission**: Broadcasts `payment.failed` AMQP event (`PAY-BR-17`).

---

## Step 3

**Downstream Execution**: Order Service cancels order (`ORD-UC-04`); Inventory Service releases stock (`INV-UC-04`).
...
sequenceDiagram
  ...
  PaySvc->>Broker: Outbox Relay publishes `payment.failed` (orderId)
  Broker->>OrderSvc: Deliver `payment.failed` -> Cancel Order
  Broker->>InvSvc: Deliver `payment.failed` -> Release Reserved Stock
```
...
|----------|----------|----------|---------|
| Payment Entity | Initial State | Handle Payment Charge Failure | Updated State |
...
|----------|------------|-------------|
| payment_records | UPDATE | Persists updated state for Handle Payment Charge Failure |
...
|----------|-----------|-------------|
| POST | /api/v1/payment/handle-payment-failure | Executes Handle Payment Charge Failure |
---
...
| Event | Description |
|---------|-------------|
| payment.failed | Emitted upon successful execution of Handle Payment Charge Failure |
---
