...
name: Process Full or Partial Order Refund

description: Processes full or partial order refund through payment gateway.
...
# Workflow: Process Full or Partial Order Refund

---

# 1. Purpose

Processes full or partial order refund through payment gateway.
...
- Core process execution for Process Full or Partial Order Refund.
...
Process Full or Partial Order Refund is initiated via incoming client API request or internal message event.
...
**State & Amount Check**: Verifies order state (`PAID`, `CANCELLED`, `RETURNED`, `PAY-BR-07`) and checks refund amount does not exceed paid total (`PAY-BR-08`, `PAY-BR-09`).

---

## Step 2

**Gateway Processing**: Calls Stripe/PayPal gateway refund API (`PAY-BR-10`).

---

## Step 3

**Ledger Update**: Creates refund record and updates transaction status to `REFUNDED` / `PARTIALLY_REFUNDED` (`PAY-BR-15`).

---

## Step 4

**Event Emission**: Emits `payment.refunded` event (`PAY-BR-18`).
...
sequenceDiagram
  autonumber
  actor Trigger as Admin / Event `order.cancelled`
  ...
  Trigger->>PaySvc: POST /api/v1/payments/refunds OR Event `order.cancelled`
  PaySvc->>DB: Query SELECT * FROM transactions WHERE order_id = ?
  PaySvc->>PaySvc: Verify Order State IN ('PAID', 'CANCELLED', 'RETURNED') (PAY-BR-07)
  PaySvc->>PaySvc: Verify Refund Amount <= Total Paid - Previous Refunds (PAY-BR-09)
  PaySvc->>GatewayProvider: Issue Gateway Refund Request (gateway_tx_id, amount)
  GatewayProvider-->>PaySvc: Refund Confirmation Receipt
  PaySvc->>DB: INSERT INTO refunds (transaction_id, amount, reason)
  PaySvc->>DB: UPDATE transactions SET status = 'REFUNDED' / 'PARTIALLY_REFUNDED'
  PaySvc->>Broker: Outbox Relay publishes `payment.refunded`
  PaySvc-->>Trigger: HTTP 200 OK (Refund Confirmed)
```
...
|----------|----------|----------|---------|
| Payment Entity | Initial State | Process Full or Partial Order Refund | Updated State |
...
|----------|------------|-------------|
| payment_records | UPDATE | Persists updated state for Process Full or Partial Order Refund |
...
|----------|-----------|-------------|
| POST | /api/v1/payments/refunds | Executes Process Full or Partial Order Refund |
---
...
| Event | Description |
|---------|-------------|
| payment.refunded | Emitted upon successful execution of Process Full or Partial Order Refund |
---
