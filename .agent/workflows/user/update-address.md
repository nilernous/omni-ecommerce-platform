---
id: WORKFLOW-USER-004

name: Update Delivery Address

description: Updates existing delivery address details in user profile.
...
# Workflow: Update Delivery Address

---

# 1. Purpose

Updates existing delivery address details in user profile.
...
- Core process execution for Update Delivery Address.
...
Update Delivery Address is initiated via incoming client API request or internal message event.
...
**Ownership Validation**: Ensures address belongs to requester (`USER-BR-17`).

---

## Step 2

**Default Handling**: If updated to default, clears existing default flags (`USER-BR-08`).

---

## Step 3

**Persistence**: Updates address details in `user_db` (`USER-BR-10`).
...
sequenceDiagram
  ...
  Client->>UserSvc: PUT /api/v1/users/addresses/{id} (Address Payload)
  ...
  alt Ownership Valid
    ...
    UserSvc->>DB: UPDATE addresses SET streetAddress = ?, ... WHERE id = ?
    UserSvc-->>Client: HTTP 200 OK (Updated Address Payload)
  else Unauthorized Owner
    UserSvc-->>Client: HTTP 403 Forbidden
  end
```
...
|----------|----------|----------|---------|
| User Entity | Initial State | Update Delivery Address | Updated State |
...
|----------|------------|-------------|
| user_records | UPDATE | Persists updated state for Update Delivery Address |
...
|----------|-----------|-------------|
| PUT | /api/v1/users/addresses/{id} | Executes Update Delivery Address |
---
