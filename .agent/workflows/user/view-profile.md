---
id: WORKFLOW-USER-001

name: View User Profile

description: Retrieves authenticated user profile information and settings.
...
# Workflow: View User Profile

---

# 1. Purpose

Retrieves authenticated user profile information and settings.
...
- Core process execution for View User Profile.
...
View User Profile is initiated via incoming client API request or internal message event.
...
**Identity Extraction**: Reads `userId` from JWT context.

---

## Step 2

**Database Query**: Fetches user record, address array, and notification preferences from `user_db`.

---

## Step 3

**Response Delivery**: Returns unified profile object (`USER-BR-01`).
...
sequenceDiagram
  autonumber
  actor Client as Customer Web / Mobile
  participant UserSvc as User Service (:3002)
  participant DB as PostgreSQL (user_db)

  Client->>UserSvc: GET /api/v1/users/profile (Bearer JWT)
  UserSvc->>UserSvc: Extract `userId` from JWT Context
  UserSvc->>DB: Query SELECT * FROM users JOIN addresses JOIN preferences WHERE id = ?
  DB-->>UserSvc: Profile Record & Associated Entities
  UserSvc-->>Client: HTTP 200 OK (User Profile Envelope)
```
...
|----------|----------|----------|---------|
| User Entity | Initial State | View User Profile | Updated State |
...
|----------|------------|-------------|
| user_records | UPDATE | Persists updated state for View User Profile |
...
|----------|-----------|-------------|
| GET | /api/v1/users/profile | Executes View User Profile |
---
