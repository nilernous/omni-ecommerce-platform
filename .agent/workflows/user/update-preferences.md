---
id: WORKFLOW-USER-006

name: Update Notification Preferences

description: Updates user communication and notification preferences.
...
# Workflow: Update Notification Preferences
...
- Core process execution for Update Notification Preferences.
...
Update Notification Preferences is initiated via incoming client API request or internal message event.
...
**Preference Ingestion**: Parses opt-in/opt-out toggle settings (`USER-BR-11`).

---

## Step 2

**Persistence**: Updates `user_preferences` table in `user_db` (`USER-BR-12`).
...
sequenceDiagram
  ...
  Client->>UserSvc: PATCH /api/v1/users/preferences (emailEnabled, marketingEnabled)
  UserSvc->>DB: UPDATE user_preferences SET marketing_enabled = ?, ... WHERE user_id = ?
  UserSvc-->>Client: HTTP 200 OK (Updated Preferences Object)
```
...
|----------|----------|----------|---------|
| User Entity | Initial State | Update Notification Preferences | Updated State |
...
|----------|------------|-------------|
| user_records | UPDATE | Persists updated state for Update Notification Preferences |
...
|----------|-----------|-------------|
| PATCH | /api/v1/users/preferences | Executes Update Notification Preferences |
---
