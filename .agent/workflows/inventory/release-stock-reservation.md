...
name: Release Expired / Cancelled Stock Reservation

description: Releases expired or cancelled stock reservations back into available inventory.
...
# Workflow: Release Expired / Cancelled Stock Reservation

---

# 1. Purpose

Releases expired or cancelled stock reservations back into available inventory.
...
- Core process execution for Release Expired / Cancelled Stock Reservation.
...
Release Expired / Cancelled Stock Reservation is initiated via incoming client API request or internal message event.
...
**Trigger Ingestion**: Responds to 15-min reservation TTL timer or `order.cancelled` event (`INV-BR-05`, `INV-BR-15`).

---

## Step 2

**Stock Release**: Decrements `reserved` balance in `inventory_db`, restoring units to available stock (`INV-BR-08`).

---

## Step 3

**Status Update**: Sets reservation status to `RELEASED` / `EXPIRED`.

---

## Step 4

**Event Emission**: Emits `inventory.released` event (`INV-BR-10`).
...
sequenceDiagram
  autonumber
  actor Trigger as Timer Worker / Order Event
  ...
  Trigger->>InvSvc: Reservation TTL Expired OR Event `order.cancelled` Received
  InvSvc->>DB: Query SELECT * FROM reservations WHERE order_id = ? AND status = 'RESERVED'
  InvSvc->>DB: BEGIN TRANSACTION: UPDATE inventory SET reserved = reserved - qty WHERE sku = ?
  InvSvc->>DB: UPDATE reservations SET status = 'RELEASED' WHERE id = ?
  DB-->>InvSvc: Transaction Committed
  InvSvc->>Broker: Outbox Relay publishes `inventory.released`
```
...
|----------|----------|----------|---------|
| Inventory Entity | Initial State | Release Expired / Cancelled Stock Reservation | Updated State |
...
|----------|------------|-------------|
| inventory_records | UPDATE | Persists updated state for Release Expired / Cancelled Stock Reservation |
...
|----------|-----------|-------------|
| POST | /api/v1/inventory/release-stock-reservation | Executes Release Expired / Cancelled Stock Reservation |
---
...
| Event | Description |
|---------|-------------|
| inventory.released | Emitted upon successful execution of Release Expired / Cancelled Stock Reservation |
---
