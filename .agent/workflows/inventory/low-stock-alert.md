...
name: Low Stock Safety Threshold Alert

description: Monitors stock levels and dispatches low-stock alerts when safety thresholds are breached.
...
# Workflow: Low Stock Safety Threshold Alert

---

# 1. Purpose

Monitors stock levels and dispatches low-stock alerts when safety thresholds are breached.
...
- Core process execution for Low Stock Safety Threshold Alert.
...
Low Stock Safety Threshold Alert is initiated via incoming client API request or internal message event.
...
**Threshold Trigger**: Fires when available quantity drops below 5 units (`INV-BR-03`).

---

## Step 2

**Event Emission**: Broadcasts `inventory.low_stock` AMQP event (`INV-BR-12`).

---

## Step 3

**Notification Consumption**: Notification Service sends alert email.
...
sequenceDiagram
  ...
  InvSvc->>InvSvc: Evaluate Available Quantity < 5 (INV-BR-03)
  InvSvc->>Broker: Publish `inventory.low_stock` (sku, availableQty)
  Broker->>Notif: Deliver `inventory.low_stock` event
  Notif->>Notif: Send email alert to merchant/admin
```
...
|----------|----------|----------|---------|
| Inventory Entity | Initial State | Low Stock Safety Threshold Alert | Updated State |
...
|----------|------------|-------------|
| inventory_records | UPDATE | Persists updated state for Low Stock Safety Threshold Alert |
...
|----------|-----------|-------------|
| POST | /api/v1/inventory/low-stock-alert | Executes Low Stock Safety Threshold Alert |
---
...
| Event | Description |
|---------|-------------|
| inventory.low_stock | Emitted upon successful execution of Low Stock Safety Threshold Alert |
---
