...
name: Query SKU Stock Balance

description: Queries available and reserved stock balances for specific product SKUs.
...
# Workflow: Query SKU Stock Balance

---

# 1. Purpose

Queries available and reserved stock balances for specific product SKUs.
...
- Core process execution for Query SKU Stock Balance.
...
Query SKU Stock Balance is initiated via incoming client API request or internal message event.
...
**Database Lookup**: Fetches physical and reserved quantity for SKU.

---

## Step 2

**Formula Calculation**: `Available Quantity = Physical Quantity - Reserved Quantity` (`INV-BR-01`).
...
sequenceDiagram
  autonumber
  actor Client as Client App / BFF
  participant InvSvc as Inventory Service (:3004)
  participant DB as PostgreSQL (inventory_db)

  Client->>InvSvc: GET /api/v1/inventory/stock/{sku}
  InvSvc->>DB: Query SELECT physical_qty, reserved_qty FROM inventory WHERE sku = ?
  DB-->>InvSvc: Stock Row
  InvSvc->>InvSvc: Compute Available = Physical - Reserved
  InvSvc-->>Client: HTTP 200 OK (Available Quantity Payload)
```
...
|----------|----------|----------|---------|
| Inventory Entity | Initial State | Query SKU Stock Balance | Updated State |
...
|----------|------------|-------------|
| inventory_records | UPDATE | Persists updated state for Query SKU Stock Balance |
...
|----------|-----------|-------------|
| GET | /api/v1/inventory/stock/{sku} | Executes Query SKU Stock Balance |
---
