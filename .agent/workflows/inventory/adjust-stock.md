...
name: Warehouse Stock Adjustment (Merchant / Admin)

description: Adjusts physical stock levels for a SKU in warehouse inventory.
...
# Workflow: Warehouse Stock Adjustment (Merchant / Admin)

---

# 1. Purpose

Adjusts physical stock levels for a SKU in warehouse inventory.
...
- Core process execution for Warehouse Stock Adjustment (Merchant / Admin).
...
Warehouse Stock Adjustment (Merchant / Admin) is initiated via incoming client API request or internal message event.
...
**Balance Update**: Adjusts physical stock balance in `inventory_db` (`INV-BR-01`).

---

## Step 2

**Audit Logging**: Inserts stock adjustment log entry.
...
sequenceDiagram
  autonumber
  actor Seller as Seller / Admin
  participant InvSvc as Inventory Service (:3004)
  participant DB as PostgreSQL (inventory_db)

  Seller->>InvSvc: POST /api/v1/inventory/adjust (sku, adjustmentQuantity, reason)
  InvSvc->>DB: UPDATE inventory SET physical = physical + adjustmentQuantity WHERE sku = ?
  InvSvc->>DB: INSERT INTO inventory_logs (sku, adjustment, reason)
  InvSvc-->>Seller: HTTP 200 OK (Updated Stock Balance Payload)
```
...
|----------|----------|----------|---------|
| Inventory Entity | Initial State | Warehouse Stock Adjustment (Merchant / Admin) | Updated State |
...
|----------|------------|-------------|
| inventory_records | UPDATE | Persists updated state for Warehouse Stock Adjustment (Merchant / Admin) |
...
|----------|-----------|-------------|
| POST | /api/v1/inventory/adjust | Executes Warehouse Stock Adjustment (Merchant / Admin) |
---
