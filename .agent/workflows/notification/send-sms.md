---
id: WORKFLOW-NOTIF-002

name: Send Transactional SMS Notification

description: Sends SMS text messages for time-sensitive notifications and verification codes.
...
# Workflow: Send Transactional SMS Notification

---

# 1. Purpose

Sends SMS text messages for time-sensitive notifications and verification codes.
...
- Core process execution for Send Transactional SMS Notification.
...
Send Transactional SMS Notification is initiated via incoming client API request or internal message event.
...
**Text Formatting**: Formats SMS text message (`NOTIF-BR-02`).

---

## Step 2

**Queue Dispatch**: Pushes SMS job to BullMQ queue (`NOTIF-BR-06`).

---

## Step 3

**Twilio Dispatch**: Worker calls Twilio API to deliver SMS to user phone number.
...
sequenceDiagram
  ...
  Broker->>NotifSvc: Deliver Event (`shipping.dispatched` / OTP Request)
  NotifSvc->>NotifSvc: Format SMS Text Message (NOTIF-BR-02)
  NotifSvc->>RedisQueue: Push SMS Job
  RedisQueue->>Twilio: Dispatch SMS (to: E.164 Phone, body)
  Twilio-->>NotifSvc: SMS Dispatched Confirmation
```
...
|----------|----------|----------|---------|
| Notification Entity | Initial State | Send Transactional SMS Notification | Updated State |
...
|----------|------------|-------------|
| notification_records | UPDATE | Persists updated state for Send Transactional SMS Notification |
...
|----------|-----------|-------------|
| POST | /api/v1/notification/send-sms | Executes Send Transactional SMS Notification |
---
