---
id: WORKFLOW-PROM-003

name: Create Promotional Coupon Code (Admin)

description: Creates promotional coupon codes with discount rules and usage limits.
...
# Workflow: Create Promotional Coupon Code (Admin)

---

# 1. Purpose

Creates promotional coupon codes with discount rules and usage limits.
...
- Core process execution for Create Promotional Coupon Code (Admin).
...
Create Promotional Coupon Code (Admin) is initiated via incoming client API request or internal message event.
...
**Persistence**: Saves coupon configuration in `promotion_db`.

---

## Step 3

**Caching**: Caches coupon key in Redis.
...
sequenceDiagram
  ...
  Admin->>PromSvc: POST /api/v1/promotions/coupons (Coupon Payload)
  PromSvc->>PromSvc: Verify Admin Role Scope
  PromSvc->>DB: INSERT INTO coupons (code, discount_type, value, start_date, end_date, caps)
  PromSvc->>Redis: SETEX coupon:{code} 86400 (Cache Coupon)
  PromSvc-->>Admin: HTTP 201 Created (Created Coupon Object)
```
...
|----------|----------|----------|---------|
| Promotion Entity | Initial State | Create Promotional Coupon Code (Admin) | Updated State |
...
|----------|------------|-------------|
| promotion_records | UPDATE | Persists updated state for Create Promotional Coupon Code (Admin) |
...
|----------|-----------|-------------|
| POST | /api/v1/promotions/coupons | Executes Create Promotional Coupon Code (Admin) |
---
