---
id: WORKFLOW-PROM-004

name: Schedule & Launch Flash Sale Campaign (Admin)

description: Launches time-bounded flash sale events with special discounted pricing.
...
# Workflow: Schedule & Launch Flash Sale Campaign (Admin)

---

# 1. Purpose

Launches time-bounded flash sale events with special discounted pricing.
...
- Core process execution for Schedule & Launch Flash Sale Campaign (Admin).
...
Schedule & Launch Flash Sale Campaign (Admin) is initiated via incoming client API request or internal message event.
...
**Campaign Creation**: Inserts flash sale campaign parameters into `promotion_db` (`PROM-BR-04`).

---

## Step 2

**Timer Scheduling**: Schedules activation worker for `startDate`.

---

## Step 3

**Event Broadcast**: Broadcasts `promotion.flash_sale_started` event upon campaign launch (`PROM-BR-12`).
...
sequenceDiagram
  ...
  Admin->>PromSvc: POST /api/v1/promotions/flash-sales (SKUs, promoPrice, stockAllocation, timestamps)
  PromSvc->>DB: INSERT INTO flash_sales (skus, promo_price, stock_limit, start_time, end_time)
  PromSvc->>PromSvc: Schedule Activation Timer Worker at start_time
  Note over PromSvc,Broker: When start_time is reached
  PromSvc->>Broker: Outbox Relay publishes `promotion.flash_sale_started`
  PromSvc-->>Admin: HTTP 201 Created
```
...
|----------|----------|----------|---------|
| Promotion Entity | Initial State | Schedule & Launch Flash Sale Campaign (Admin) | Updated State |
...
|----------|------------|-------------|
| promotion_records | UPDATE | Persists updated state for Schedule & Launch Flash Sale Campaign (Admin) |
...
|----------|-----------|-------------|
| POST | /api/v1/promotions/flash-sales | Executes Schedule & Launch Flash Sale Campaign (Admin) |
---
...
| Event | Description |
|---------|-------------|
| promotion.flash_sale_started | Emitted upon successful execution of Schedule & Launch Flash Sale Campaign (Admin) |
---
