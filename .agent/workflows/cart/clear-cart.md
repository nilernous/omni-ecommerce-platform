---
id: WORKFLOW-CART-006

name: Clear Cart Upon Order Placement

description: Wipes all items from user cart in Redis upon order placement or explicit request.
...
# Workflow: Clear Cart Upon Order Placement

---

# 1. Purpose

Wipes all items from user cart in Redis upon order placement or explicit request.
...
- Core process execution for Clear Cart Upon Order Placement.
...
Clear Cart Upon Order Placement is initiated via incoming client API request or internal message event.
...
**Event Trigger**: Receives `order.placed` AMQP event (`CART-BR-13`).

---

## Step 2

**Key Deletion**: Deletes Redis key `cart:{userId}` (`CART-BR-09`).

---

## Step 3

**Event Emission**: Emits `cart.cleared` event (`CART-BR-11`).
...
sequenceDiagram
  ...
  Broker->>CartSvc: Deliver Event `order.placed` (userId)
  CartSvc->>Redis: DEL cart:{userId}
  CartSvc->>Broker: Publish `cart.cleared`
```
...
|----------|----------|----------|---------|
| Cart Entity | Initial State | Clear Cart Upon Order Placement | Updated State |
...
|----------|------------|-------------|
| cart_records | UPDATE | Persists updated state for Clear Cart Upon Order Placement |
...
|----------|-----------|-------------|
| POST | /api/v1/cart/clear-cart | Executes Clear Cart Upon Order Placement |
---
...
| Event | Description |
|---------|-------------|
| cart.cleared | Emitted upon successful execution of Clear Cart Upon Order Placement |
---
