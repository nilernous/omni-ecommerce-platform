---
id: WORKFLOW-AUTH-007

name: Account Lockout Handling

description: Enforces security protection by tracking failed login attempts and locking accounts after threshold failures.
...
# Workflow: Account Lockout Handling

---

# 1. Purpose

Enforces security protection by tracking failed login attempts and locking accounts after threshold failures.
...
- Core process execution for Account Lockout Handling.
...
Account Lockout Handling is initiated via incoming client API request or internal message event.
...
**Failure Counting**: Upon invalid password attempt, Auth Service increments Redis failure key `login_failures:{ip}:{email}` with 15-minute window TTL.

---

## Step 2

**Threshold Evaluation**: Checks if failure count reaches 5 (`AUTH-BR-11`).

---

## Step 3

**Lockout Trigger**: Creates Redis key `lockout:{ip}:{email}` with 900-second TTL (15 mins).

---

## Step 4

**Request Rejection**: Subsequent login attempts during lockout window immediately return HTTP 429 without querying database.
...
sequenceDiagram
  autonumber
  actor Client as Unauthenticated Client / Attacker
  participant Auth as Auth Service (:3001)
  participant Redis as Redis (session_cache)

  Client->>Auth: POST /api/v1/auth/login (Invalid Credentials Attempt)
  Auth->>Redis: INCR login_failures:{ip}:{email}
  Redis-->>Auth: Count (e.g. 5)
  alt Count == 5
    Auth->>Redis: SETEX lockout:{ip}:{email} 900 "locked"
    Auth-->>Client: HTTP 429 Too Many Requests (Account Locked for 15 Min)
  else Count < 5
    Auth-->>Client: HTTP 401 Unauthorized (Invalid Credentials)
  end
```
...
|----------|----------|----------|---------|
| Authentication Entity | Initial State | Account Lockout Handling | Updated State |
...
|----------|------------|-------------|
| auth_records | UPDATE | Persists updated state for Account Lockout Handling |
...
|----------|-----------|-------------|
| POST | /api/v1/auth/login | Executes Account Lockout Handling |
---
