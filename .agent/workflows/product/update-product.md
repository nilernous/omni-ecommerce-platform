...
description: Updates existing product listing attributes, pricing, or media assets.
...
# Workflow: Update Product Specification

---

# 1. Purpose

Updates existing product listing attributes, pricing, or media assets.
...
- Core process execution for Update Product Specification.
...
Update Product Specification is initiated via incoming client API request or internal message event.
...
**Validation**: Validates pricing and SKU rules (`PRD-BR-03`, `PRD-BR-04`).

---

## Step 2

**Persistence**: Updates product record in `product_db`.

---

## Step 3

**Cache Purge**: Deletes Redis key `product:detail:{id}` (`PRD-BR-18`).

---

## Step 4

**Event Emission**: Emits `product.updated` event (`PRD-BR-14`).
...
sequenceDiagram
  ...
  Seller->>PrdSvc: PUT /api/v1/products/{id} (Update Payload)
  PrdSvc->>DB: UPDATE products SET title = ?, base_price = ? WHERE id = ?
  PrdSvc->>Redis: DEL product:detail:{id}
  PrdSvc->>DB: INSERT INTO product_outbox (event: product.updated)
  PrdSvc->>Broker: Outbox Relay publishes `product.updated`
  PrdSvc-->>Seller: HTTP 200 OK (Updated Product)
```
...
|----------|----------|----------|---------|
| Product Entity | Initial State | Update Product Specification | Updated State |
...
|----------|------------|-------------|
| product_records | UPDATE | Persists updated state for Update Product Specification |
...
|----------|-----------|-------------|
| PUT | /api/v1/products/{id} | Executes Update Product Specification |
---
...
| Event | Description |
|---------|-------------|
| product.updated | Emitted upon successful execution of Update Product Specification |
---
