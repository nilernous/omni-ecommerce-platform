...
name: Browse & Filter Product Catalog

description: Retrieves filtered and paginated product listings for storefront browsing.
...
# Workflow: Browse & Filter Product Catalog

---

# 1. Purpose

Retrieves filtered and paginated product listings for storefront browsing.
...
- Core process execution for Browse & Filter Product Catalog.
...
Browse & Filter Product Catalog is initiated via incoming client API request or internal message event.
...
**Cache Inspection**: Checks Redis for cached list key matching filter parameters (`PRD-BR-17`).

---

## Step 2

**Database Query**: On cache miss, queries `product_db` for items with `status: ACTIVE` (`PRD-BR-10`).

---

## Step 3

**Response Envelope**: Formats paginated collection envelope and caches result in Redis (15-min TTL).

---

## Step 4

**Client Response**: Delivers catalog products array to client.
...
sequenceDiagram
  ...
  Client->>Gateway: GET /api/v1/products?category=electronics&page=1
  Gateway->>PrdSvc: Forward Request
  PrdSvc->>Redis: GET catalog:list:electronics:page1
  alt Cache Hit
    Redis-->>PrdSvc: Cached List Response Payload
  else Cache Miss
    PrdSvc->>DB: Query SELECT * FROM products WHERE status = 'ACTIVE' AND category_id = ?
    DB-->>PrdSvc: Active Products & Paginated Total Count
    PrdSvc->>Redis: SETEX catalog:list:electronics:page1 900 (15 min TTL)
  end
  PrdSvc-->>Gateway: HTTP 200 OK (Paginated Catalog Envelope)
  Gateway-->>Client: Return Products List
```
...
|----------|----------|----------|---------|
| Product Entity | Initial State | Browse & Filter Product Catalog | Updated State |
...
|----------|------------|-------------|
| product_records | UPDATE | Persists updated state for Browse & Filter Product Catalog |
...
|----------|-----------|-------------|
| GET | /api/v1/products?category=electronics&page=1 | Executes Browse & Filter Product Catalog |
---
