...
name: Create Product Category Hierarchy (Admin)

description: Creates new catalog product category for organizing storefront items.
...
# Workflow: Create Product Category Hierarchy (Admin)

---

# 1. Purpose

Creates new catalog product category for organizing storefront items.
...
- Core process execution for Create Product Category Hierarchy (Admin).
...
Create Product Category Hierarchy (Admin) is initiated via incoming client API request or internal message event.
...
**Depth Check**: Verifies nested level is `<= 3 levels deep` (`PRD-BR-07`).

---

## Step 2

**Persistence**: Saves category record in `product_db`.
...
sequenceDiagram
  ...
  Admin->>PrdSvc: POST /api/v1/products/categories (name, parentId)
  PrdSvc->>DB: Check Category Depth Level (SELECT depth FROM categories WHERE id = parentId)
  alt Depth >= 3
    PrdSvc-->>Admin: HTTP 400 Bad Request (MAX_CATEGORY_DEPTH_EXCEEDED)
  else Depth < 3
    PrdSvc->>DB: INSERT INTO categories (name, parent_id, depth)
    PrdSvc-->>Admin: HTTP 201 Created (Created Category Object)
  end
```
...
|----------|----------|----------|---------|
| Product Entity | Initial State | Create Product Category Hierarchy (Admin) | Updated State |
...
|----------|------------|-------------|
| product_records | UPDATE | Persists updated state for Create Product Category Hierarchy (Admin) |
...
|----------|-----------|-------------|
| POST | /api/v1/products/categories | Executes Create Product Category Hierarchy (Admin) |
---
