---
id: WORKFLOW-SHIP-002

name: Generate Carrier Shipping Label

description: Generates carrier shipping label for order fulfillment.
...
# Workflow: Generate Carrier Shipping Label

---

# 1. Purpose

Generates carrier shipping label for order fulfillment.
...
- Core process execution for Generate Carrier Shipping Label.
...
Generate Carrier Shipping Label is initiated via incoming client API request or internal message event.
...
**Order Status Check**: Verifies order status is `PAID` (`SHIP-BR-04`).

---

## Step 2

**Carrier Booking**: Requests label booking from carrier API, assigning tracking number (`SHIP-BR-05`).

---

## Step 3

**Media Upload**: Uploads label PDF to object storage via Media Service (`SHIP-BR-06`).

---

## Step 4

**Persistence**: Saves shipment record in `shipping_db` with status `LABEL_CREATED` (`SHIP-BR-09`).

---

## Step 5

**Event Emission**: Emits `shipping.label_created` event (`SHIP-BR-15`).
...
sequenceDiagram
  ...
  participant MediaSvc as Media Service (:3011)
  participant DB as PostgreSQL (shipping_db)
  participant Broker as RabbitMQ Event Bus

  Seller->>ShipSvc: POST /api/v1/shipping/shipments (orderId, carrierCode)
  ShipSvc->>ShipSvc: Verify Order Status == PAID (SHIP-BR-04)
  ShipSvc->>CarrierAPI: Request Booking & Shipping Label (orderId, carrierCode)
  CarrierAPI-->>ShipSvc: Tracking Number (tx_999) & Label PDF Binary
  ShipSvc->>MediaSvc: POST /upload (Label PDF)
  MediaSvc-->>ShipSvc: Label CDN URL
  ShipSvc->>DB: INSERT INTO shipments (order_id, tracking_number, status: LABEL_CREATED, label_url)
  ShipSvc->>Broker: Outbox Relay publishes `shipping.label_created`
  ShipSvc-->>Seller: HTTP 201 Created (Shipment Record & Label PDF URL)
```
...
|----------|----------|----------|---------|
| Shipping Entity | Initial State | Generate Carrier Shipping Label | Updated State |
...
|----------|------------|-------------|
| shipping_records | UPDATE | Persists updated state for Generate Carrier Shipping Label |
...
|----------|-----------|-------------|
| POST | /api/v1/shipping/shipments | Executes Generate Carrier Shipping Label |
---
...
| Event | Description |
|---------|-------------|
| shipping.label_created | Emitted upon successful execution of Generate Carrier Shipping Label |
---
