---
id: WORKFLOW-SRCH-002

name: Auto-Complete Search Suggestions

description: Provides fast real-time search autocomplete suggestions for query terms.
...
api:
  - GET /api/v1/search/suggestions?q=hea
...
# Workflow: Auto-Complete Search Suggestions

---

# 1. Purpose

Provides fast real-time search autocomplete suggestions for query terms.
...
- Core process execution for Auto-Complete Search Suggestions.
...
Auto-Complete Search Suggestions is initiated via incoming client API request or internal message event.
...
**Length Check**: Checks query string length `>= 2 chars` (`SRCH-BR-04`).

---

## Step 2

**ES Completion**: Executes completion suggestion query against Elasticsearch `products_index`.

---

## Step 3

**Response**: Returns top 5 suggested keywords.
...
sequenceDiagram
  autonumber
  actor Client as Search Bar
  participant SrchSvc as Search Service (:3102)
  participant ES as Elasticsearch (products_index)

  Client->>SrchSvc: GET /api/v1/search/suggestions?q=hea
  alt q.length < 2
    SrchSvc-->>Client: HTTP 200 OK (Empty Array [])
  else q.length >= 2
    SrchSvc->>ES: Execute Edge N-Gram Completion Query (q, limit = 5)
    ES-->>SrchSvc: Matching Completion Phrases
    SrchSvc-->>Client: HTTP 200 OK (Top 5 Keyword Suggestions)
  end
```
...
|----------|----------|----------|---------|
| Search Entity | Initial State | Auto-Complete Search Suggestions | Updated State |
...
|----------|------------|-------------|
| search_records | UPDATE | Persists updated state for Auto-Complete Search Suggestions |
...
|----------|-----------|-------------|
| GET | /api/v1/search/suggestions?q=hea | Executes Auto-Complete Search Suggestions |
---
